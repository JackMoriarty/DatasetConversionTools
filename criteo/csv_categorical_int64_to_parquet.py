...
categorical_field = [pa.field(item, pa.int64()) for item in CATEGORICAL_COLUMNS]
...
categorical_default_values = {item : 0 for item in CATEGORICAL_COLUMNS}
default_values = {}
...
categorical_converters={x:partial(int, base=16) for x in CATEGORICAL_COLUMNS}
...
for i, df in enumerate(pd.read_csv(src_file, \
                                   ...
                                   names=INPUT_COLUMNS,
                                   dtype=input_dtype,
                                   converters=categorical_converters)):
